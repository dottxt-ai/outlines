...
def chat_completion(
    ...
    *,
    ...
    max_tokens: Optional[int] = None,
    temperature: Optional[float] = None,
) -> Tuple[Callable, List[Tuple[str, str]]]:
    """Call language models with a chat interaction pattern.
    ...
    model_path
        A string of the form "model_provider/model_name"
    prefix
        The prefix used at the beginning of the prompt, called "system" in the
    ...
    """
    llm_builder = routers.chat_completion(model_path)
    chat_llm = llm_builder(
        stop_at=stop_at, max_tokens=max_tokens, temperature=temperature
    )

    init_state = []
    if prefix is not None:
        init_state.append(("prefix", prefix))
    ...
    def model(
        ...
        We adopt a unifying API for all chat completion models:

        - `instruction` represents the "instructions" passed to the model before
        the first user query. Called "system" with OpenAI's ChatGPT, the prompt
        that precedes the first `Human:` occurence with Anthropic's Claude for
        instance.
        - `history` represents the sequence of user queries and model return
          values, respectively indexed by "user" and "model".
        - `query` represents the current user query.

        Note
        ----

        As Claude's documentation shows, chat APIs are nothing more than an
        abstraction over the recursive prompt completion that often happens in
        language model workflows. It is in fact possible to pass models
        compatible with `chat_completion` to the `completion` function, which is
        equivalent to call this API with `system=None` and `history=None`.
        ...
        state
            The current state of the completion, which represents the history of
            interactions with the model, a list of tuples that contain the
            message and its author.

        Returns
        -------
        The return value of the model and the state updated with the user query and
        the model's return value.

        """
        state.append(("user", query))
        result = chat_llm(query, state)
        state.append(("model", result))
        return result, state
