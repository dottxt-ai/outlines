...
from outlines.inputs import Image, Chat, AssistantMessage
...
class GeminiTypeAdapter(ModelTypeAdapter):
    ...
    def format_tools(
        self, tools: Optional[List[ToolDef]] = None
    ) -> Optional[list]:
...
class Gemini(Model):
    ...
    def generate_stream(
        ...
        **inference_kwargs
        ...
        -------
        Iterator[AssistantMessage]
            An iterator that yields the AssistantMessage generated by the model.
        ...
        for chunk in stream:
            yield self._handle_gemini_stream_chunk(chunk)

    def _handle_gemini_response(self, response) -> AssistantMessage:
        """Convert the response from the Gemini API to an AssistantMessage.
    ...
    def _handle_gemini_stream_chunk(self, chunk) -> AssistantMessage:
        """Convert the streaming chunk from the Gemini API to an
        AssistantMessage.
            ...
            if hasattr(candidate, "content") and candidate.content:
                for part in candidate.content.parts:
                    if hasattr(part, "function_call") and part.function_call:
                        ...
                        return AssistantMessage(content=part.text)
