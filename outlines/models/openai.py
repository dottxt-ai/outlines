...
class OpenAITypeAdapter(ModelTypeAdapter):
    ...
    def format_output_type(self, output_type):
        ...
        if isinstance(output_type, Regex):
            raise TypeError(
                "Neither regex-based structured outputs nor the `pattern` keyword in Json Schema are not available with OpenAI. Use an open source model or dottxt instead."
            )
