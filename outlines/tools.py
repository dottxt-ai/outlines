...
from outlines.types.dsl import is_pydantic_model, is_callable, is_dict_instance
...
class StreamingToolCall(TypedDict):
    tool_call_id: str
    name: str
    args: str
...
def get_formatted_tools(
    ...
    for tool in tools:
        if is_dict_instance(tool):
            ...
            formatted_tools.append(_callable_to_tool_def(tool))

        elif is_pydantic_model(tool):
            formatted_tools.append(_pydantic_model_to_tool_def(tool))
...
def _type_to_string(type_annotation: Any) -> str:
    ...
    if type_annotation is str:
        ...
        return str(type_annotation).lower()
