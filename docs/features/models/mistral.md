...
- Structured outputs (including Pydantic/dataclass-based ones) are unavailable for codestral-* models, because they lack JSON Schema support.
...
- Structured outputs may be **unavailable** for `codestral-*` models (they lack JSON schema support).
...
Streaming methods (`.stream()` / `.stream_async()`) allow you to iterate over tokens as they are generated.
...
```python
# Sync
results = model.generate("Respond with one word.", n=3)
assert isinstance(results, list)
assert len(results) == 3

# Async
results = await model.generate("Respond with one word.", n=3)
assert isinstance(results, list)
assert len(results) == 3
```
