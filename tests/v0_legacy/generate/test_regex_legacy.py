...
def test_regex_legacy_openai(openai_model):
    with pytest.warns(
        DeprecationWarning,
        match="The `regex` function is deprecated",
    ):
        with pytest.raises(
            NotImplementedError,
            match="Cannot use regex-structured generation with an OpenAI model"
            + "due to the limitations of the OpenAI API.",
        ):
            regex(openai_model, REGEX_STRING)


def test_regex_legacy_transformers_vision(transformers_vision_model):
    with pytest.warns(
        DeprecationWarning,
        match="The `regex` function is deprecated",
    ):
        generator = regex(transformers_vision_model, REGEX_STRING)
    ...
    assert isinstance(generator.generator.logits_processor, RegexLogitsProcessor)


def test_regex_legacy_standard_model(transformers_model):
    with pytest.warns(
        DeprecationWarning,
        match="The `regex` function is deprecated",
    ):
        generator = regex(transformers_model, REGEX_STRING, samplers.greedy())
