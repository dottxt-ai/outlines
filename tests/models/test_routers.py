...
def test_language_model_invalid_provider():
    with pytest.raises(ValueError, match="model provider"):
        routers.language_completion("xx/model_name")


def test_language_model_router():
    dummy_model_name = "model_name"
    llm_builder = routers.language_completion(f"openai/{dummy_model_name}")
    assert llm_builder.func == models.OpenAICompletion
    assert llm_builder.args == (dummy_model_name,)

    llm_builder = routers.language_completion(f"hf/{dummy_model_name}")
