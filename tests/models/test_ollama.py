...
def test_wrong_inference_parameters():
    with pytest.raises(TypeError, match="got an unexpected"):
        Ollama(CLIENT, MODEL_NAME).generate("Respond with one word. Not more.", None, foo=10)
...
def test_ollama_simple():
    result = Ollama(CLIENT, MODEL_NAME).generate("Respond with one word. Not more.", None)
...
def test_ollama_json():
    ...
    result = Ollama(CLIENT, MODEL_NAME)("Respond with one word. Not more.", JsonType(Foo))
    assert isinstance(result, str)
